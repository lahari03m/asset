...
# Clean issue names
cleaned_issues = {
    (issue if not issue.startswith("nan") else "Scheduled Maintenance"): count
    for issue, count in asset_details['issues'].items()
}

most_common_issue = max(
    {k: v for k, v in cleaned_issues.items() if k != "Scheduled Maintenance"},
    key=cleaned_issues.get,
    default="No Issues"
)
...
scheduled_maintenance_count = cleaned_issues.get("Scheduled Maintenance", 0)
if scheduled_maintenance_count:
    st.info(f"🛠️ **Scheduled Maintenance Count:** {scheduled_maintenance_count}")

# --- Issues Breakdown (Excluding Scheduled Maintenance) ---
st.markdown("### 🔍 Issue Distribution (Excluding Scheduled Maintenance)")

filtered_issues = {issue: count for issue, count in cleaned_issues.items() if issue != "Scheduled Maintenance"}

if filtered_issues:
    issues_df = pd.DataFrame(list(filtered_issues.items()), columns=['Issue Description', 'Count'])
    fig_issues = px.bar(issues_df, x='Count', y='Issue Description', orientation='h',
                        title=f"Issues Frequency for {selected_asset} (Excluding Scheduled Maintenance)",
                        color='Count', color_continuous_scale='Viridis')
    st.plotly_chart(fig_issues, use_container_width=True)
else:
    st.info("No issues recorded for this asset apart from Scheduled Maintenance.")
